...
import random
import DecisionNCE
from TransitionNCE import Transition_NCE_loss
...
from TransitionNCE import Transition_NCE #这个环境里pip install -e .是原版bearobot库，所以V_network直接写相对路径，不然找不到
...
v_model = Transition_NCE(state_dim=9).to("cuda")
...
class LIBEROEval_err(BaseEval):
       ...
       def _rollout(self, policy: BaseAgent, task_id: int=0, img_goal=False):
              ...
              for t in tqdm(range(self.eval_horizon), desc=f'{lang}'):
                     ...
                     # imgs = image_input.squeeze(1) # [B, V, C, H, W]
                     if self.image_initial == None:
                            self.image_initial = image_1
                     
                     state_seq.append(state.permute(0,1))
                     state_seq_tensor = torch.stack(state_seq,dim=1)
                     # if len(state_seq) < 10:
                     #        continue 
                     # else:
                     ...
                     # img_transition, P_state_transition, N_state_transition = v_model(self.image_initial,image_1,state_seq_tensor,N_state)
                     state_seq_e, text_e = v_model(state_seq_tensor,[lang])
                     last_state_e = state_seq_e[:,-1,:]
                     
                     reward = Transition_NCE_loss()
                     reward_matrix = reward.Transition_Reward_diff(state_seq_e,text_e)
                     print(reward_matrix)
                     # b = cosine_similarity(last_state_e, text_e, dim=-1)
                     # print(b)
                     # b_list.append(b.cpu().detach().numpy())
                     b_list.append(reward_matrix.cpu().detach().numpy())
...
def load_ckpt(agent,policy_ckpt_path):
...
def build_visual_diffsuion_err(policy_ckpt_path: str,v_model_ckpt_path: str,statistics_path: str, k: float=0.2, num_episodes: int=10 ,wandb_name: str=None, wandb_path: str=None):
       ...
       return load_ckpt(agent,policy_ckpt_path)
